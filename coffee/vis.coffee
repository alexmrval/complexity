...
Network = () ->
  ...
  update = () ->
    ...
    # radial layout
    if layout == "radial"
      areas = sortedArtists(curNodesData, curLinksData)
      updateCenters(areas)
  ...
  network.updateSearch = (searchTerm) ->
    ...
    node.each (d) ->
      ...
      match = d.area.toLowerCase().search(searchRegEx)
      if searchTerm.length > 0 and match >= 0
        ...
        d.searched = false
        element.style("fill", (d) -> nodeColors(d.area))
  ...
  setupData = (data) ->
    ...
    countExtent = d3.extent(data.nodes, (d) -> d.complexity)
    ...
    data.nodes.forEach (n) ->
      ...
      n.radius = circleRadius(n.complexity)
  ...
  # attr is value stored in node, like 'area'
  ...
  # Helper function to set line colour based on complexity
  ...
  filterNodes = (allNodes) ->
    ...
    if filter == "popular" or filter == "obscure"
      complexities = allNodes.map((d) -> d.complexity).sort(d3.ascending)
      cutoff = d3.quantile(complexities, 0.5)
  ...
  # Returns array of areas sorted based on
  ...
  sortedArtists = (nodes,links) ->
    ...
    if sort == "links"
      counts = {}
      links.forEach (l) ->
        counts[l.source.area] ?= 0
        counts[l.source.area] += 1
        counts[l.target.area] ?= 0
        counts[l.target.area] += 1
      # add any missing areas that dont have any links
      nodes.forEach (n) ->
        counts[n.area] ?= 0

      # sort based on counts
      areas = d3.entries(counts).sort (a,b) ->
        b.value - a.value
      # get just names
      areas = areas.map (v) -> v.key
    else
      # sort areas by song count
      counts = nodeCounts(nodes, "area")
      areas = d3.entries(counts).sort (a,b) ->
        b.value - a.value
      areas = areas.map (v) -> v.key
  ...
  updateCenters = (areas) ->
    if layout == "radial"
      groupCenters = RadialPlacement().center({"x":width/2, "y":height / 2 - 100})
        .radius(300).increment(18).keys(areas)
  ...
  updateNodes = () ->
    ...
    node.enter().append("circle")
      ...
      .attr("cy", (d) -> d.y)
      .attr("r", (d) -> d.radius)
      .style("fill", (d) -> nodeColors(d.area))
  ...
  updateLinks = () ->
    link = linksG.selectAll("line.link")
      .data(curLinksData, (d) -> "#{d.source.id}_#{d.target.id}_#{d.link_complexity}")
    link.enter().append("line")
      .attr("class", "link")
      .attr("stroke", (d) -> linecolor(d.link_complexity))
  ...
  moveToRadialLayout = (alpha) ->
    k = alpha * 0.1
    (d) ->
      centerNode = groupCenters(d.area)
  ...
  # particular node.
  strokeFor = (d) ->
    d3.rgb(nodeColors(d.area)).darker().toString()
    legend[d.area] = d3.rgb(nodeColors(d.area)).darker().toString()

  # Mouseover tooltip function
  showDetails = (d,i) ->
    content = '<p class="main">' + d.area + '</span></p>'
    tooltip.showTooltip(content,d3.event)

    # higlight connected links
    if link
        link.attr("stroke-opacity", (l) ->
          if l.source == d or l.target == d then 1.0 else 0.1
        )
  ...
  hideDetails = (d,i) ->
    ...
    if link
      link.attr("stroke", (d) -> linecolor(d.link_complexity))
        .attr("stroke-opacity", 0.6)

  # Display legend function

  showLegend = () ->
    id = document.getElementById("legend")
    id.innerHTML = "Functional areas: </br>"
    for val, key of legend
      id.innerHTML += "<i class=\"fa fa-circle\" style=color:#{key};padding:5px; aria-hidden=\"true\">#{val}</i>"
...
$ ->
  ...
  d3.json "data/daybreak.json", (json) ->
    myNetwork("#vis", json)
