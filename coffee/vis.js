...
(function() {
  ...
  Network = function() {
    ...
    setupData = function(data) {
      ...
      data.nodes.forEach(function(n) {
        ...
        return n.radius = circleRadius(n.complexity);
      });
      nodesMap = mapNodes(data.nodes);
      data.links.forEach(function(l) {
        l.source = nodesMap.get(l.source);
        l.target = nodesMap.get(l.target);
        l.coupling = nodesMap.get(l.link_complexity);
    ...
    filterNodes = function(allNodes) {
      var complexities, cutoff, filteredNodes;
      filteredNodes = allNodes;
      if (filter === "popular" || filter === "obscure") {
        complexities = allNodes.map(function(d) {
          return d.complexity;
        }).sort(d3.ascending);
        cutoff = d3.quantile(complexities, 0.5);
        filteredNodes = allNodes.filter(function(n) {
          if (filter === "popular") {
            return n.complexity > cutoff;
    ...
    sortedArtists = function(nodes, links) {
      ...
      if (sort === "links") {
        ...
        links.forEach(function(l) {
          var name, name1;
          if (counts[name = l.source.area] == null) {
            counts[name] = 0;
          }
          counts[l.source.area] += 1;
          if (counts[name1 = l.target.area] == null) {
            counts[name1] = 0;
          }
          return counts[l.target.area] += 1;
        });
        nodes.forEach(function(n) {
          var name;
          return counts[name = n.area] != null ? counts[name] : counts[name] = 0;
        });
        areas = d3.entries(counts).sort(function(a, b) {
          return b.value - a.value;
        });
        areas = areas.map(function(v) {
          return v.key;
        });
      } else {
    ...
    updateLinks = function() {
      link = linksG.selectAll("line.link").data(curLinksData, function(d) {
        return d.source.id + "_" + d.target.id + "_" + d.link_complexity;
      });
      if (link.link_complexity === "high") {
        link.enter().append("line").attr("class", "link").attr("stroke", "#ff0000").attr("stroke-opacity", 0.8).attr("x1", function(d) {
          return d.source.x;
        }).attr("y1", function(d) {
          return d.source.y;
        }).attr("x2", function(d) {
      ...
      } else if (link.link_complexity === "med") {
        link.enter().append("line").attr("class", "link").attr("stroke", "#00ff00").attr("stroke-opacity", 0.8).attr("x1", function(d) {
          return d.source.x;
        }).attr("y1", function(d) {
          return d.source.y;
        }).attr("x2", function(d) {
        ...
        return link.exit().remove();
      } else if (link.link_complexity === "low") {
        link.enter().append("line").attr("class", "link").attr("stroke", "#0000ff").attr("stroke-opacity", 0.8).attr("x1", function(d) {
          return d.source.x;
        }).attr("y1", function(d) {
          return d.source.y;
        }).attr("x2", function(d) {
    ...
    setLayout = function(newLayout) {
      layout = newLayout;
      if (layout === "force") {
        return force.on("tick", forceTick).charge(-200).linkDistance(200);
    ...
    strokeFor = function(d) {
      return d3.rgb(nodeColors(d.area)).darker().toString();
    };
  ...
  $(function() {
    ...
    return d3.json("data/daybreak.json", function(json) {
